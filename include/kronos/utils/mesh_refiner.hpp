...
public:
    ...
    // 构建首次迭代的物理初值猜测 (对齐 Python 端的两点线性插值)
    static std::vector<double> build_initial_guess() {
        ...
        std::vector<double> x0_val = {
            1.0 + 100000.0/R0, 0.0, 0.0, 7450.0/V_ref, -0.5*PI/180.0, 0.0, 0.0, tf_guess
        };
        std::vector<double> xf_val = {
            1.0 + 25000.0/R0, 12.0*PI/180.0, 70.0*PI/180.0, 2000.0/V_ref, -10.0*PI/180.0, 90.0*PI/180.0, 0.0, tf_guess
        };
        ...
        for (int k = 0; k < N + 1; ++k) {
            ...
            // 2. 添加配点变量 W_k
            if (k < N) {
                // 配点内部状态 Xc (大小 d * nx)：使用当前主节点平铺
                for (int j = 0; j < d; ++j) {
                    guess.insert(guess.end(), x_curr.begin(), x_curr.end());
                }
                // 配点控制量 Uc (大小 d * nu)：[u_rate, slack_Q, slack_q, slack_n] 使用 0 平铺
                for (int j = 0; j < d; ++j) {
                    guess.insert(guess.end(), {0.0, 0.0, 0.0, 0.0});
                }
            }
        }
        return guess;
    ...
    static RefineResult compute(const std::vector<double>& old_fractions, const std::vector<double>& flat_solution) {
        ...
        // 计算网格误差 Monitor function (依据 V 和 gamma 的导数跃变)
        ...
        for (int k = 0; k < N; ++k) {
            ...
            // 【关键修复】：内部配点状态必须用主节点状态填充，不能用零/极小值
            for (int j = 0; j < d; ++j) {
                new_flat_x0.insert(new_flat_x0.end(), x_interp.begin(), x_interp.end());
            }
            // 内部控制量可用零填充
            for (int j = 0; j < d; ++j) {
                new_flat_x0.insert(new_flat_x0.end(), {0.0, 0.0, 0.0, 0.0});
            }
        }
