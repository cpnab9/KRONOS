# offline_codegen/generate_adaptive.py
import casadi as cs
import numpy as np
import os
...
for k in range(K):
    if k < K - 1:
        # 【修改】：dt_k 的计算依赖于参数 mesh_fractions
        tf_var = x[k][7]
...
print("[*] 开始生成参数化 KRONOS NLP 与 连续动力学 f_cont C代码 ...")

# 【核心导出 1】：导出连续动力学 f_cont 供 C++ 计算误差用
f_cont.generate('f_cont.c', {"with_header": True})

# 【核心导出 2】：导出带网格参数 p 和初值猜测 x0 的求解器对象
opti.to_function("kronos_nlp", 
                 [mesh_fractions, opti.x], [opti.f, opti.x], 
                 ["p", "x0"], ["f", "x_opt"]).generate('nlp_code.c', {"with_header": True})

# 动态生成用于 C++ 展平输出和重网格的元数据
...
print(f"✅ 参数化自适应代码生成成功！文件保存在: {generated_dir}")